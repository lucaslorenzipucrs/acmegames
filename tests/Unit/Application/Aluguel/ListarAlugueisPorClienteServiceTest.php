...
use App\Application\Aluguel\Services\ListarAlugueisPorClienteService;
...
class ListarAlugueisPorClienteServiceTest extends TestCase
{
    public function testListaAlugueisPorClienteComSucesso()
    {
        ...
        $repo->expects($this->once())->method('listarPorCliente')->with(1)->willReturn([$aluguel]);

        $service = new ListarAlugueisPorClienteService($repo);

        $resultado = $service->executar(1);
